...
data_long$Method <- factor(data_long$Method, levels = desired_order)
 ...
 plot <- ggplot(data_long, aes(x = Method, y = Value, fill = Category)) +
   ...
   labs(y = "Count") +
   theme_minimal() +
   theme(axis.text.x = element_text(angle = 45, hjust = 1, size=13), axis.text.y = element_text(size = 10),
   ...
   annotate("text", x = 4, y = -4000, label = "Reference guided", vjust = 0.5, size = 6) +
   annotate("text", x = 8, y = -4000, label = "De novo", vjust = 0.5, size = 6, fontface = 'italic') +
